...
#include "../../../modules/task_2/asoskov_m_sobel/sobel.h"
...
Sobel::Sobel(size_t m_size) {
  ...
  for (size_t i = 0; i < m_size * m_size; i++) matrix[i] = 0;
}

Sobel::Sobel(const Sobel& c) {
  mSize = c.mSize;
  matrix = new int[mSize * mSize];
  for (size_t i = 0; i < mSize * mSize; i++) matrix[i] = c.matrix[i];
}
...
bool Sobel::operator==(const Sobel& c) const {
  for (size_t i = 0; i < mSize * mSize; i++)
    if (matrix[i] != c.matrix[i]) return false;
  return true;
}
...
Sobel Sobel::operator_Sobel(Sobel c) {
  ...
  for (int i = 1; i < _size; i++)
    for (int j = 1; j < _size; j++) {
      ...
      int range = val < min_pix ? min_pix
                  : val > max_pix ? max_pix : val;
